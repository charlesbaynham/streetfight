...
function NearestPlayers({ shot_data }) {
  ...
  const userIndex = context.findIndex((location) => location.user_id === shooting_user_id);
  ...
  const otherUsersContext = context.filter((location) => location.user_id !== shooting_user_id);
  ...
  const shooting_users = otherUsersContext.map(({ user_id, team_id, user, team, latitude, longitude, state, timestamp }) => {
    ...
    return { distance, user_id, team_id, user, team, latitude, longitude, state, timestamp };
  }
  );
