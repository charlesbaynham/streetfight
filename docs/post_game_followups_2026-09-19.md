...
## 13. Mystery boxes, weighted by how badly you are doing

**Idea.** From talking to players afterwards: rubber-band mechanics, to help
people who are lagging catch up. The proposed shape is a **question box** in
the drops - a mystery item that, when scanned, plays an animation scrolling
through a a bunch of possible items and lands on one at random, with the
probabilities adjusted by score: better items when you are doing badly, worse
when you are doing well.

**What already fits.** The plumbing is the same well-trodden path as items 11
and 12 - a new `ItemType`, payload, validator, handler, artwork - and this one
belongs in `SINGLE_ARTWORK_TYPES` for the same reason `radar` and
`circle_warning` do: there is one drawing, because what varies is the outcome
and the picture cannot say it. The animation has a precedent too:
`TemporaryOverlay`, which `BulletCount.js` already uses for the bullet,
armour, medkit and gun pickups. A scrolling reel is more than that, but it
lives in the same place. Any sound it needs is generated by
`scripts/make_sounds.py` rather than recorded.

**The roll has to happen on the server, and that is a good fit.** A printed
code is an HMAC over its payload and nothing else - which is exactly what
decouples printing from deploying - so a mystery box *cannot* carry its own
outcome: it would be fixed at print time and identical on every copy of that
card. The payload carries a tier or a table name; `item_actions` rolls when
the code is collected. The frontend's reel is then a presentation of an
answer the server already gave, not the draw itself.

**The sharp one: a refusal is a reroll.** Several handlers deliberately
refuse and roll the whole scan back so the card survives to be used later -
`start_radar` while a radar is running, `start_circle_warning` with no next
circle placed, `_handle_armour` for armour no better than you are wearing.
That is a good bargain for a card whose outcome is *known*. For a random one
it is an exploit: roll a radar while yours is running, the handler raises, the
scan rolls back, no `Item` row is written - and the player scans the same box
again for a fresh roll until they like the answer. Any random item therefore
has to either never delegate to a handler that can refuse, settle the roll
before the refusable part, or record the roll against the card so a rescan
replays the same outcome rather than drawing again.

**"Score" exists, but it measures the wrong thing.**
`AdminInterface.get_scoreboard` computes a score as the sum of `shot_damage`
over a player's *checked* shots that hit somebody - damage dealt, derived per
request rather than stored. That is a measure of **offence**, not of
hardship: a player who has been knocked out three times and has no ammo left
can have a perfectly good score, and a lucky early shot can carry somebody
who has done nothing since. Deciding what "lagging behind" means is the first
question here, and the state that speaks to it more directly is already on
the `User` row - `hit_points`, `num_bullets`, the weapon held
(`shot_damage` / `shot_timeout`), `appeals_remaining` - with knockouts
suffered computable from the shots aimed at them. Note also that the
scoreboard's score only counts **checked** shots, so a player's standing lags
the queue: during a showdown (items 9 and 10) it can be minutes stale, which
is exactly when a catch-up mechanic would be firing.

**Two design consequences worth settling before building it.**

*Whether players are told it is weighted.* If it is, and they work it out -
and thirty people playing for hours will - the rational move is to sandbag:
underperform deliberately to farm better boxes. That is a real behavioural
risk of the mechanic, not a hypothetical, and it is decided by how strong the
weighting is and whether it is secret.

*The ethics check this repo already asks for.* `CLAUDE.md` routes anything
that steers a player's choice to the **`nudge-ux` agent** and
`docs/nudge/ethics.md`, and a loot box with a scrolling reel is the one place
this game would borrow a gambling interface wholesale. That is very likely
fine - a private game, no money, friends who came out on a Saturday - but it
is exactly the check the repository says to make, so make it deliberately
rather than by omission. The same library's warning applies to the claim as
well: published nudge effects are overstated by roughly six times
(`docs/nudge/evidence.md`), and at thirty players whether rubber-banding
*worked* will not be measurable. Build it because it makes the night more
fun to be losing at, not because the numbers will show anything.

**Cheapest first step, if it is wanted at all.** A mystery box with a *flat*
table - no weighting - is the whole mechanic minus the hard question: it
needs the new item type, the server-side roll, the reroll defence and the
animation, and it tests whether the box is fun before anybody has to define
"doing badly". The weighting can be added later by changing one function.
