...
| 3     | **#12** Redraw the Westminster map          | **Shipped 17 Sept** | Play area fixed, drawn map active, resort venue retired. #7 unblocked. **Reopened 12 Sept:** the final nineteen-pub list replaced the ten the map was traced from, so eleven pubs have no marker. **Shipped 17 Sept (M0.6):** rendered, not generated. Every Gemini image model and every image-output model on OpenRouter was tried and each got either the roads or the scale wrong - image models resynthesise rather than trace. `render_venue_map.py` draws the OSM geometry with a wobbly pen and a handwriting font instead: exact by construction, all nineteen pubs labelled, free to re-run when the list changes. The map poster (M0.7) is unblocked. |
...
**Half done (M5, 17 Sept).** The sounds are all generated by
`scripts/make_sounds.py` now, so re-tuning one is an edit to the function that
made it. `hit_received.wav` and `bang.wav` were rewritten there - the first was
a 130 Hz thump that a coat pocket ate whole, which is a poor showing for the
one sound in the game a player cannot afford to miss - and `knocked_out.wav`
and `shot_knockout.wav` are new. `shot_confirmed.wav` and `shot_missed.wav`
are still the original placeholder tones: they are fine, and they are the two
that arrive while the player is already looking at the screen.
